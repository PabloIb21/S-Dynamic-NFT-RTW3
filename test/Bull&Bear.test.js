...
const { moveBlocks, moveTime } = require("../utils");
...
before(async () => {
  ...
  await vrfCoordinatorMock.addConsumer(subscriptionId, tokenContract.address);
});
...
describe("Test Bull&Bear", () => {
  ...
  it("Upkeep correctly updates Token URIs on consecutive price increases", async () => {
    ...
    expect(newTokenUri).to.include("_bull.json");
